...
const COPY = {
  es: {
    ...
    micro: "MVP activo · Lanzamiento comercial agosto 2026.",
    ...
    duplicate: "Ya estás en la lista. Te avisamos cuando KAIRON esté listo.",
  },
  en: {
    ...
    micro: "Live MVP · Commercial launch August 2026.",
    ...
    duplicate: "You’re already on the list. We’ll let you know when KAIRON is ready.",
  },
} as const;
...
export function WaitlistInline({ source = "home", submitLabel, compact = false }: WaitlistInlineProps) {
  ...
  if (status === "success") {
    return (
      <div className={`border border-foreground bg-[color:var(--color-brand-soft)]/40 ${compact ? "p-5" : "p-7 md:p-8"}`}>
  ...
  return (
    <form
      onSubmit={onSubmit}
      className={`border border-border bg-[color:var(--color-surface)] ${compact ? "p-5" : "p-7 md:p-8"}`}
    >
      <label htmlFor={`waitlist-email-${source}`} className="eyebrow">
        {t.label}
      </label>
